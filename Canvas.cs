    ...
    public class Canvas
    {
        ...
        public void Render(Scene scene)
        {
            ...
            for (int f = 0; f < scene.Figures.Count; f++)
            {
                ...
                {
                    g.FillPolygon(Brushes.Blue, scene.Figures[f].Pts.ToArray());
                    g.DrawPolygon(Pens.Green, scene.Figures[f].Pts.ToArray());
        ...
        //------------------------------------------------------------------------Render modificado
        //Solo se ejecuta para figuras de más de dos puntos
        public void Render2(Scene scene, int fnum)
        {
            FastClear();

            if (scene.RFigures[fnum].Pts.Count > 1)
                {
                    g.FillPolygon(Brushes.Red, scene.RFigures[fnum].Pts.ToArray());
                    g.DrawPolygon(Pens.White, scene.RFigures[fnum].Pts.ToArray());
                    g.FillEllipse(Brushes.Violet, scene.RFigures[fnum].Pts[scene.RFigures[fnum].Pts.Count - 1].X - 3, scene.RFigures[fnum].Pts[scene.RFigures[fnum].Pts.Count - 1].Y - 3, 6, 6);
                    g.FillEllipse(Brushes.Yellow, scene.RFigures[fnum].Centroid.X - 3, scene.RFigures[fnum].Centroid.Y - 3, 6, 6);//*/
                }
            pct.Invalidate();
        }
